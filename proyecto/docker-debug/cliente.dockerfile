...
WORKDIR /proyecto/practica3/CodigoEsqueleto/raft/cmd/clientraft

# Compilar el código Go asegurándose de que puede encontrar el archivo go.mod
RUN go build -o clientraft .

# Etapa final: imagen ligera para ejecutar el binario
FROM alpine:latest

# Copiar el binario compilado desde la etapa de construcción
COPY --from=builder /proyecto/practica3/CodigoEsqueleto/raft/cmd/clientraft/clientraft /proyecto/practica3/CodigoEsqueleto/raft/cmd/clientraft/clientraft
